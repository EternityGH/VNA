    ...
    return (
        <View style={[styles.container, {paddingHorizontal: 30,paddingTop: 110}]}>
            ...
            <View style={[styles.groupText,{justifyContent: 'space-between', marginTop: 24}]}>
                <TouchableOpacity onPress={()=>navigation.navigate('ForgotPassword')}><Text style={[styles.linkText, {color: '#07ad87'}]}>{checkLanguage({vi: 'Quên mật khẩu?', en: `Forgot password?`},language)}</Text></TouchableOpacity>
                <TouchableOpacity onPress={()=>navigation.replace('Reg')}><Text style={[styles.linkText], {color: '#07ad87'}}>{checkLanguage({vi: 'Đăng ký', en: `Register`},language)}</Text></TouchableOpacity>
            </View>
